...
#define TAG "de.appwerft.sru"

using namespace v8;

static Persistent<Object> bindingCache;

static void Sru_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Sru.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::SruBindings::lookupGeneratedInit(
...
static void Sru_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Sru_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Sru_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::SruBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry SruBinding = {
	"de.appwerft.sru",
	Sru_init,
	Sru_dispose
};
...
Java_de_appwerft_sru_SruBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.sru", &SruBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::SruBindings::lookupGeneratedInit));
}
